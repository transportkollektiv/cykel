...
def start_rents_for_reservations():
    upcoming_reservations = Reservation.objects.filter(rent__isnull=True, bike__isnull=True, event__start__lt=now()+timedelta(hours=2), event__end__gt=now())
