...
def get_forbidden_reservation_time_ranges(day: Day, vehicle_type: VehicleType):
    ...
    for occurrence_to_check in occurrences:
        # https://django-scheduler.readthedocs.io/en/latest/periods.html
        # #classify-occurrence-occurrence
        ...
        for occurrence in occurrences:
            # ignore self
            if (
                occurrence_to_check["occurrence"].event.id
            ...
            if occurrence_to_check["class"] == 0 or occurrence_to_check["class"] == 1:
                if occurrence["class"] == 2:
                    ...
                    if (
                        occurrence_to_check["occurrence"].event.start
                        >= occurrence["occurrence"].event.start
                    ) and (
                        occurrence_to_check["occurrence"].event.start - lead_time_delta
                        < occurrence["occurrence"].event.end
                    ):
                        number_of_start_in_other_reservations += 1
                        if (occurrence["class"] == 1 or occurrence["class"] == 3) and (
                            occurrence["occurrence"].event.end < forbidden_range_end
                        ):
                            forbidden_range_end = occurrence["occurrence"].event.end
        if (
            number_of_start_in_other_reservations + 1
            >= number_of_bikes - vehicle_type.min_spontaneous_rent_vehicles
        ):
            ...
            forbidden_range_ends_in_next_day = tomorrow <= (
                forbidden_range_end + lead_time_delta
            )
            ...
            forbidden_range_start = make_naive(
                occurrence_to_check["occurrence"].event.start
            )
            yesterday = make_naive(day.start) - timedelta(days=1)
            forbidden_range_starts_yesterday = (
                yesterday.date() >= (forbidden_range_start - lead_time_delta).date()
            )
            forbidden_range_start_time = (
                forbidden_range_start - lead_time_delta
            ).time()
            ...
            forbidden_range = {
                "start": forbidden_range_start_time,
                "end": forbidden_range_end_time,
            }
            forbidden_ranges.append(forbidden_range)

    if (
        all_occurrences_go_entire_day
        and len(occurrences)
        >= number_of_bikes - vehicle_type.min_spontaneous_rent_vehicles
    ):
...
def is_allowed_reservation_day(day: Day, vehicle_type: VehicleType):
    ...
    for occurrence in occurrences:
        ...
        start_time_with_delta = (
            start_time - timedelta(minutes=vehicle_type.reservation_lead_time_minutes)
        ).date()
        end_time = make_naive(occurrence["occurrence"].end)
        end_time_with_delta = (
            end_time + timedelta(minutes=vehicle_type.reservation_lead_time_minutes)
        ).date()
        if day.start.date() < start_time.date():
            ...
            if not (
                (start_time.date() > start_time_with_delta)
                and (end_time.date() < end_time_with_delta)
            ):
                number_of_occ -= 1
...
def get_maximum_reservation_date(start_date_time: datetime, vehicle_type: VehicleType):
    ...
    for day_delta in range(max_reservation_days + 1):
        day_to_check = start_date_time + timedelta(
            days=day_delta
        )  # 0 - max reservation time
        ...
        if forbidden_ranges:
            for forbidden_range in forbidden_ranges:
                # ignore forbidden range if it ends before reservation starts,
                # can only happen on first day
                if day_delta == 0:
            ...
            if forbidden_ranges_relevant:
                max_reservation_date = day_to_check.replace(
